...
class Noun(tk.Frame):
    ...
    result. However, the correctness of the result cannot be ensured. """).pack()
        ...
        def generate(display=True):
            ...
            def inputerror(arg1=None, arg2=None, arg3=None, arg4=None, arg5=None):
                ...
                text.insert(0, 'mensa,mensae')
                text.config(state='readonly')
                text.grid(row=0, column=1)
                temp = tk.StringVar()
                menu = tk.OptionMenu(example, temp, 'feminine', )
                temp.set('feminine')
                menu.config(width=8)
    ...
    Please check your input ").pack()
            ...
            def first(nom, gen, gender):
                ...
                apply_to_all(fill_in_the_blank, blank_name, answer)
            ...
            if display:
                self.build()
                blank_name = [self.nom_s, self.gen_s, self.dat_s, self.acc_s, self.abl_s, self.voc_s, self.nom_p,
                              self.gen_p, self.dat_p, self.acc_p, self.abl_p, self.voc_p]
        ...
        tk.Label(self, text="e.g. mensa, mensae, feminine").grid(row=1, columnspan=2)
    ...
    def build(self):
        ...
        #result_panel.wm_protocol('WM_DELETE_WINDOW',lambda: (f for f in (self.result_panel.destroy(),self.window_widget.deiconify())))
        ...
        tk.Label(result_panel, text='Declension of the word: ' + self.data.get() + ', ' + self.gender.get(),
                 font=('Times New Roman', 20, 'italic')).grid(row=0, columnspan=3)

        tk.Label(result_panel, text='singular').grid(row=1, column=1, )
        tk.Label(result_panel, text='plural').grid(row=1, column=2, )

        ###left column###
        tk.Label(result_panel, text="nominative").grid(row=2, column=0)
        tk.Label(result_panel, text="genitive").grid(row=3, column=0)
        tk.Label(result_panel, text="dative").grid(row=4, column=0)
        tk.Label(result_panel, text="accusative").grid(row=5, column=0)
        tk.Label(result_panel, text="ablative").grid(row=6, column=0)
        tk.Label(result_panel, text="vocative").grid(row=7, column=0)

        ###singular
        self.nom_s = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.nom_s.grid(row=2, column=1, )
        self.gen_s = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.gen_s.grid(row=3, column=1, )
        self.dat_s = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.dat_s.grid(row=4, column=1, )
        self.acc_s = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.acc_s.grid(row=5, column=1, )
        self.abl_s = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.abl_s.grid(row=6, column=1, )
        self.voc_s = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.voc_s.grid(row=7, column=1, )

        ###plural###
        self.nom_p = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.nom_p.grid(row=2, column=2, )
        self.gen_p = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.gen_p.grid(row=3, column=2, )
        self.dat_p = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.dat_p.grid(row=4, column=2, )
        self.acc_p = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.acc_p.grid(row=5, column=2, )
        self.abl_p = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.abl_p.grid(row=6, column=2, )
        self.voc_p = tk.Entry(result_panel, width=answer_width, state='readonly')
        self.voc_p.grid(row=7, column=2, )
        tk.Label(result_panel, text='This word belongs to ' + self.action[self.declension][0] + ' declension', \
                 font=('Times New Roman', 16)).grid(row=8, columnspan=3, )
        tk.Button(result_panel, text='Back', command=lambda: (f for f in (result_panel.destroy(),self.window_widget.deiconify()))).grid(row=9, columnspan=3, )
